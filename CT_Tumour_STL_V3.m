...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic;
TumourMargin = 3; %% Safety margin. Keep it 2 mm less than advised margin, numerical error will take care of.
...
theta= linspace(0,2*pi,100); delta_theta = 45; % degree
...
for longitude = 0:delta_theta:(360 - delta_theta)
    ...
    for latitude = start_lat:delta_theta:end_lat
        ...
        [StatesOfAllNineTines, area, VerticalAblationLimits, PredictedAblation] = OverlappingFromDatabase3D(NiceDataCutPlane(:,1:2)',userProfile,fig1);
        ...
        if size(StatesOfAllNineTines,1) ~= 0
            ...
            Flag = IsTumourCoveredByAblation(tri_long_lat,PredictedAblation,TumourMargin);
            if Flag
                figure(fig1);plot(alphaShape(PredictedAblation,20));axis image;
                PlotShiftedAndRotatedTumour(tri_long_lat,fig1); hold off;
                disp('ADB ^3');
            else
                % figure;plot(NiceDataCutPlane(:,1),NiceDataCutPlane(:,2),'r');axis image;
                figure(fig1);plot(alphaShape(PredictedAblation,20));axis image;
                PlotShiftedAndRotatedTumour(tri_long_lat,fig1); hold off;
                %close(gcf);
