...
%% for all the triangels that are intersected by zee;) slice
...
t1 = (c-sum(P.*p1))./sum(P.*(p1-p2)); % ratio of vertical distance between point(p1) and the slice to vertical distance between the points(p1 & p2)
t2 = (c-sum(P.*p2))./sum(P.*(p2-p3)); %% same
t3 = (c-sum(P.*p3))./sum(P.*(p3-p1)); %% same
...
imain = i1+i2+i3 == 2; % avoid collinear points;

pts_out = [[intersect1(:,i1&i2&imain);intersect2(:,i1&i2&imain)],[intersect2(:,i2&i3&imain);...
    intersect3(:,i2&i3&imain)], [intersect3(:,i3&i1&imain);intersect1(:,i3&i1&imain)]]; % true(1) will access the correct intersections
